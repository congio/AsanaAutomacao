...
PROJECT_GID = os.getenv("PROJETO_GID")
...
try:
    ...
    while True:
        ...
        for task in api_response:
            ...
            # Determina a situação baseada na posição da seção e status de conclusão isso varia de projeto para projeto
            # "Realizado" (valor padrão)
            situacao = "Realizado"  # Valor padrão
            
            if section_info['position'] == 1:  # ESTUDO PRÉVIO
                situacao = "A Realizar"
            elif 2 <= section_info['position'] <= 14 or (section_info['position'] == 16): # Seções 2-15
    ...
    output_file = "NOME_DO_PROJETO_tarefas.xlsx"
    output_path = "\\SERVIDOR\Dashboard\Fonte de dados interna\NOME_DO_PROJETO_tarefas.xlsx"
